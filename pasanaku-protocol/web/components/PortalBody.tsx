    ...
    <div className="rise">
      <section className="grid items-end gap-10 lg:grid-cols-12">
        <div className="lg:col-span-7">
          <p className="inline-flex items-center gap-2 rounded-full bg-mint px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-num">
            <span className="live-dot h-1.5 w-1.5 rounded-full bg-num" />
            Key válida
          </p>
          <h1 className="mt-7 font-display text-[2.5rem] font-semibold leading-[1.05] sm:text-5xl">Sala del gremio</h1>
          <p className="mt-5 max-w-xl font-serif text-[1.55rem] italic leading-snug text-ink/75">
            Acá se lee lo que el contrato no explica en una tx.
          </p>
          <p className="mt-5 max-w-xl text-[15.5px] leading-7 text-dim">
            Portal token-gated. Sin Key Unlock no existe. Adentro: reglamento, reputación, seguro y
            cómo se parte el 6%. El pozo no pasa por nosotros.
          </p>
        </div>
        <figure className="lg:col-span-5">
          <img src="/gremio-taller.png" alt="Taller del gremio" className="photo h-56 w-full sm:h-72" />
      ...
      <section className="mt-14 grid gap-4 md:grid-cols-12">
        <article className="glow rounded-[22px] border border-line bg-card p-6 md:col-span-6">
          <p className="text-[11px] uppercase tracking-[0.2em] text-dim">Reputación</p>
          <p className="font-num mt-4 text-5xl text-num">{rep.padStart(2, "0")}</p>
        ...
        <article className="glow rounded-[22px] border border-line bg-card p-6 md:col-span-6">
          ...
          <p className="font-num mt-4 text-5xl text-num">{seguro}</p>
          <p className="mt-3 text-[15px] leading-6 text-dim">
      ...
      <section className="mt-16 grid items-center gap-10 lg:grid-cols-12">
        <div className="lg:col-span-6">
          <p className="kicker">Compra / activación</p>
          <h2 className="mt-2 font-display text-[1.75rem] font-semibold">1 USDC, no el pozo</h2>
          <p className="mt-4 text-[15.5px] leading-7 text-dim">
        ...
        <figure className="lg:col-span-6">
          <img src="/gremio-pozo.png" alt="Pozo compartido" className="photo h-64 w-full" />
        </figure>
      </section>

      <section className="mt-16">
        <p className="kicker">Reglamento</p>
        <h2 className="mt-2 font-display text-[1.75rem] font-semibold">Quién cobra qué</h2>
        <p className="mt-3 max-w-2xl text-[15.5px] leading-7 text-dim">
          En crédito, el del turno no aporta esa semana. Fee 1% + seguro 0,30%. Los primeros dejan un
          recorte al bono del que cierra (~6% de un pozo, repartido).
        </p>
        <div className="mt-8 grid gap-4 lg:grid-cols-2">
          <Compare
            n="01"
            t="Primero"
            d="Cobra ya. Se lleva el pozo menos recortes. Paga el 6% con los demás tempranos."
            bars={[
              { k: "Pozo neto", w: "78%" },
              { k: "Fee + seguro", w: "13%" },
              { k: "Bono último", w: "9%" },
          ...
          <Compare
            n="cierra"
            t="Último"
            d="Espera. Recibe pozo + bono acumulado + colateral. Por eso el paciente gana."
            bars={[
              { k: "Pozo neto", w: "87%" },
              { k: "Bono ~6%", w: "100%" },
              { k: "Colateral", w: "100%" },
      ...
      <section className="mt-16">
        <p className="kicker">Entrada y salida</p>
        <h2 className="mt-2 font-display text-[1.75rem] font-semibold">Cómo entra y sale la plata</h2>
...
function Rule({ n, t, d }: { n: string; t: string; d: string }) {
  return (
    <li className="rounded-[22px] border border-line bg-card p-6 transition duration-300 hover:-translate-y-0.5">
...
function Compare({
  ...
  return (
    <article className={`rounded-[22px] border border-line p-6 ${last ? "bg-mint" : "bg-card"}`}>
